...
export class Hex {
    ...
    public interpolateLinearly(b: Hex, t: number): Hex {
        return new Hex(this.q * (1.0 - t) + b.q * t, this.r * (1.0 - t) + b.r * t, this.s * (1.0 - t) + b.s * t);
    }

    public drawLine(b: Hex): Hex[] {
        ...
        for (var i = 0; i <= N; i++) {
            results.push(a_nudge.interpolateLinearly(b_nudge, step * i).round());
        }
        return results;
    }
...
export class OffsetCoord {
    ...
    public static qoffsetToCube(offset: number, h: OffsetCoord): Hex {git@github.com:LOBsTerr/hex.git
        var q: number = h.col;
